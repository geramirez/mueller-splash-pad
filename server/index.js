...
class DBStatusRepository {
  ...
  async getStatus(parkName) {
    ...
    const { workingVotes, notWorkingVotes } = lastVoteRecords.reduce(
      (acc, record) => {
...
const calculateVoteWeight = (parkLocation, location) => {
  ...
  if (arePointsNear(location, parkLocation, 0.2)) return 2;
  return 1;
};
...
app.post("/status/:parkKey", async (req, res) => {
  ...
  if (location) {
    ...
    if (vote === "on")
      await statusRepository.addOnVote(
        calculateVoteWeight(parkLocation, location),
        parkKey
      );
    else if (vote === "off")
      await statusRepository.addOffVote(
        calculateVoteWeight(parkLocation, location),
        parkKey
      );
  }
